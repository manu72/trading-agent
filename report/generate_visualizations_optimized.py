...
# Reduce the DPI and figure size to save memory
DPI = 150  # Lower DPI
FIGSIZE_STANDARD = (10, 6)  # Smaller figure size
...
def create_momentum_visualization():
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/momentum_performance.png', dpi=DPI, bbox_inches='tight')
    plt.close()
...
def create_sector_rotation_heatmap():
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/sector_rotation_heatmap.png', dpi=DPI, bbox_inches='tight')
    plt.close()
...
def create_swing_trading_visualization():
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/swing_trading_pattern.png', dpi=DPI, bbox_inches='tight')
    plt.close()
...
def create_risk_management_tree():
    ...
    for node, (x, y) in nodes.items():
        plt.scatter(x, y, s=800, color=node_colors[node], edgecolors='black', zorder=10)
    ...
    for node, (x, y) in nodes.items():
        plt.annotate(node_labels[node], (x, y), ha='center', va='center', fontsize=8, fontweight='bold')
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/risk_management_tree.png', dpi=DPI, bbox_inches='tight')
    plt.close()
...
def create_crypto_volatility_comparison():
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/crypto_volatility.png', dpi=DPI, bbox_inches='tight')
    plt.close()
...
def create_system_architecture():
    ...
    # Define the components with simplified layout
    components = {
        'kb': (5, 8, 2, 1.5, 'Trading Knowledge Base'),
        'agent': (5, 6, 2, 1.5, 'AI Trading Agent'),
        'algo': (5, 4, 2, 1.5, 'Algorithmic Trading System'),
        'data': (2, 6, 2, 1.5, 'Data Analysis & Monitoring'),
        'ib_api': (8, 4, 2, 1.5, 'Interactive Brokers API'),
        'market': (8, 2, 2, 1.5, 'Financial Markets'),
        'kaizen': (8, 8, 2, 1.5, 'Continuous Improvement')
    }
    ...
    # Draw the connections with simplified styling
    for start, end, label in connections:
        ...
        plt.text(mid_x + offset_x, mid_y + offset_y, label, ha='center', va='center',
                fontsize=7, bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="gray", alpha=0.8))
    
    # Add title and remove axes
    plt.title('DeepResearch Trading System Architecture', fontsize=16)
    plt.axis('off')
    
    # Add legend for component types (simplified)
    legend_elements = [
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/system_architecture.png', dpi=DPI, bbox_inches='tight')
    plt.close()
...
def create_implementation_timeline():
    ...
    for week in range(0, 25, 2):
        plt.text(week, -0.5, str(week), ha='center', va='center', fontsize=8)
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/implementation_timeline.png', dpi=DPI, bbox_inches='tight')
    plt.close()
...
def create_position_sizing_visualization():
    ...
    plt.text(0.5, 0.05, formula, ha='center', va='center', fontsize=12,
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/position_sizing.png', dpi=DPI, bbox_inches='tight')
    plt.close()

# 9. KPI Dashboard
def create_kpi_dashboard():
    # Create a figure with subplots
    fig, axs = plt.subplots(2, 2, figsize=FIGSIZE_STANDARD)
    ...
    for i, v in enumerate(returns):
        ax1.text(i, v + 0.3 if v > 0 else v - 0.7, f'{v}%', ha='center', fontsize=7)
    ...
    ax3.set_title('Risk Metrics', fontsize=12)
    ax3.set_xticks(x)
    ...
    for i, v in enumerate(values):
        ax3.text(i - width/2, v + 0.1 if v > 0 else v - 0.7, f'{v}', ha='center', fontsize=7)
    ...
    ax4.set_title('Current Asset Allocation', fontsize=12)
    ...
    plt.suptitle('Trading Strategy Performance Dashboard', fontsize=14, y=0.98)
    
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/kpi_dashboard.png', dpi=DPI, bbox_inches='tight')
    plt.close()
...
def create_ai_decision_process():
    ...
    # Define the components and their positions (simplified)
    components = [
        (1, 6, 'Market Data\nCollection', '#c6dbef'),
        (3, 6, 'Feature\nEngineering', '#9ecae1'),
        (5, 6, 'Pattern\nRecognition', '#6baed6'),
    ...
    # Define the connections (simplified)
    connections = [
        (0, 1), (1, 2), (2, 3), (3, 4),  # Main flow
        (0, 5), (1, 6), (2, 7), (3, 8),  # Downward connections
        (5, 9), (6, 9), (7, 10), (8, 10), (8, 11),  # Lower level connections
        (9, 12), (10, 12), (11, 12),  # Feedback connections
        (12, 2), (12, 3)  # Feedback to upper levels
    ]
    
    # Draw the components
    for i, (x, y, label, color) in enumerate(components):
        circle = plt.Circle((x, y), 0.7, facecolor=color, edgecolor='black', alpha=0.8)
    ...
    for start, end in connections:
        ...
        start_x += 0.7 * np.cos(angle)
        start_y += 0.7 * np.sin(angle)
        end_x -= 0.7 * np.cos(angle)
        end_y -= 0.7 * np.sin(angle)
    ...
    # Add title and remove axes
    plt.title('AI Trading Agent Decision Process', fontsize=16)
    plt.axis('off')
    
    # Add legend for process stages
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#c6dbef', markersize=10, label='Data Collection'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#9ecae1', markersize=10, label='Processing'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#6baed6', markersize=10, label='Analysis'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#4292c6', markersize=10, label='Prediction'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#2171b5', markersize=10, label='Decision'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#fdae61', markersize=10, label='Learning')
    ]
    ...
    # Save the figure
    plt.savefig('/home/ubuntu/trading_system/report/images/ai_decision_process.png', dpi=DPI, bbox_inches='tight')
    plt.close()

# Generate visualizations one by one to avoid memory issues
print("Generating momentum visualization...")
create_momentum_visualization()

print("Generating sector rotation heatmap...")
create_sector_rotation_heatmap()

print("Generating swing trading visualization...")
create_swing_trading_visualization()

print("Generating risk management tree...")
create_risk_management_tree()

print("Generating crypto volatility comparison...")
create_crypto_volatility_comparison()

print("Generating system architecture diagram...")
create_system_architecture()

print("Generating implementation timeline...")
create_implementation_timeline()

print("Generating position sizing visualization...")
create_position_sizing_visualization()

print("Generating KPI dashboard...")
create_kpi_dashboard()

print("Generating AI decision process diagram...")
